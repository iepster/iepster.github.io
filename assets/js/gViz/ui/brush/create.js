    ...
    if (validate(step)) {
      ...
      switch (step) {
        ...
        case 'run':
          ...
          _var.wrap = _var.wrap.enter().append("svg").attr('class', `diwo-visual-brush chart-${_var._id}`).merge(_var.wrap); // svg
          ...
          // Draw gB for brush
          _var.g = _var.wrap.selectAll("g.brush-wrap").data(["brush-wrap"]); // svg:g
          _var.g.exit().remove();
          _var.g = _var.g.enter().append('g').attr('class', "brush-wrap").merge(_var.g);
