      ...
      switch (step) {
        ...
        case 'run':
          ...
          bar
            ...
            .attr('x', -_var.barWidth/2)
            .attr('y', function(d) { return _var.getY(d) + _var.barWidth/2; })
            .attr('width', _var.barWidth)
            .attr('height', function(d) { return _var.getHeight(d) - _var.barWidth; })
            .style("fill", "url(#diwo-gradient-"+_var._id+"-color-"+gradId+")")
            //.style("fill", function(d) { return node.color; })
            .style("opacity", 1)

          // Update Symbols
          var symbol = nodeSel.selectAll('.symbol.element').data([_var.getY(node), _var.getY(node) + _var.getHeight(node) - _var.barWidth]);
          symbol.exit().remove();
          ...
          symbol
            ...
            .attr("d", function(d) {
              return "M " + (0) + " " + (d) +
                     "L " + (_var.barWidth/2) + " " + (d + _var.barWidth/2) +
                     "L " + (0) + " " + (d + _var.barWidth) +
                     "L " + (-_var.barWidth/2) + " " + (d + _var.barWidth/2) +
                     "Z ";
            })
