...
module.exports = function () {
  ...
  let components = {
    ...
    xScaleTop: require('./x-scale-top.js'),
  ...
  let _id = `vis-bar-positive-negative-deviation-${Math.floor(Math.random() * ((1000000000 - 5) + 1)) + 5}`;
  let _var = null;
  let action = 'build';
  let animation = 900;
  let container = null;
  ...
  let data = [];
  let height = null;
  let margin = { top: 30, right: 2, bottom: 20, left: 0 };
  let width = null;
  let positiveRange = '0 to 50';
  let negativeRange = '0 to 50';
  ...
  let validate = function (step) {
    switch (step) {
      ...
      case 'xScaleTop':     return data != null && data.data != null && data.data.length > 0;
  ...
  ['_id', '_var', 'action', 'animation', 'container', 'colors', 'data', 'height', 'margin', 'width', 'positiveRange', 'negativeRange'].forEach(function (key) {
